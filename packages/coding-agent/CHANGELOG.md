...
- Fixed an unhandled `EPIPE` rejection still crashing OMP at MCP-server startup on Windows when a stdio server exits mid-handshake. The `StdioTransport` `notify` / `#sendResponse` fix in 15.8.1 only caught synchronous `FileSink` throws; on Windows, Bun's streaming writer surfaces broken pipes as an asynchronously rejected `Promise` returned from `write()` / `flush()` once a prior successful write has engaged it, and that rejection escaped the sync `try/catch` as a fatal `unhandledRejection`. `request()` was also bypassing the shared helper entirely. `writeFrame()` now awaits any thenable returned from `write()`/`flush()` (funneling sync and async failures into one `false` return), and `request()` routes through it — failed writes reject the in-flight request with `Transport closed while sending request "<method>"` and tear the transport down instead of leaking ([#1782](https://github.com/can1357/oh-my-pi/issues/1782)).
